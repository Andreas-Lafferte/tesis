...
Los resultados de los modelos multinivel son presentados en la Tabla \@ref(tab:model). La correlación intraclase (ICC) para el modelo nulo (no mostrado) es igual a 0.217, lo cual indica la cantidad de varianza de $PSCi$ que puede atribuirse a la estructura de agrupación en la población, en este caso, los países [ICC en base a @hoxMultilevelAnalysisTechniques2017a, p.13]. Esto significa que cerca de un 22% de la varianza total del conflicto percibido por los individuos se asocia a características específicas de los países, asentando como precedente que existen diferencias significativas entre estos respecto al nivel de percepción de conflicto.
...
capmo <- "Modelos de regresión multinivel para las percepciones de conflicto social en 33 países entre 1999-2019"

texreg::htmlreg(l = list(model_2.e, model_3.e, model_4.e, model_6.e),
                    ...
                    groups = list("<b>Efectos fijos</b>" = 1, "Posición de clase (ref: 1.Capitalistas)" = 2:9,
                      ...
                      "Identificación política (ref: Derecha)" = 17:19, "<b>Interacciones entre niveles</b> (ref: 1.Capitalistas $\\times$ Ratio 80/20)" = 22:29),stars = c(0.01,0.05,0.1), custom.gof.rows = list("<b>Efectos aleatorios</b>" = c(" ", " ", " ", " "), "<b>Ajuste modelo</b>" = c(" ", " ", " ", " "), "$\\tau_{00}$ (varianza nivel país)" = c("0.89", "2.23", "2.49", "2.67"), "$\\sigma^2$ (varianza nivel individual)" = c("3.26", "3.22", "3.19", "3.18")),
...
g6 <- ggpredict(model_6, terms = c("CLASS", "C_RATIO [minmax]")) %>% 
...
Para ilustrar esta relación, en la Figura \@ref(fig:fig10) se grafican los valores predichos de $PSCi$ para cada categoría de clase según los niveles mínimos y máximos de la concentración del ingreso.[^15] En términos sustantivos, cuatro elementos pueden notificarse. Primero, en países con baja desigualdad económica no existen mayores diferencias entre las posiciones de clase respecto a la percepción de conflicto ya que, como se observa, las variaciones en el patrón son mínimas. Segundo, en países con alta desigualdad económica si se pueden observar leves diferencias entre las categorías de clase respecto al valor esperado en la escala de $PSCi$. Particularmente, los pequeños empleadores, los expertos directivos y los expertos sin autoridad presentan diferencias significativas que constatan con los coeficientes del Modelo 4. Tercero, en contextos de alta desigualdad estas tres categorías de clase tienden a percibir menores conflictos sociales que los individuos pertenecientes a la clase capitalista. Y cuarto, los individuos de clases subordinadas no perciben mayores conflictos que los individuos de clase capitalista cuando la desigualdad aumenta sino que, al contrario, perciben menos tensiones sociales.

### Ajuste comparativo

En relación al ajuste de los modelos, en la Tabla \@ref(tab:fit) se presentan los estadísticos del análisis de devianza (likelihood ratio test) incluyendo los modelos 1 al 4 y el modelo nulo. La prueba de razón de verosimilitud se utiliza para comparar modelos anidados y determinar cuál se ajusta mejor a los datos. De acuerdo con esta prueba, se puede notificar que los modelos más complejos incrementan significativamente el ajuste. Tomando como ejemplo el Modelo nulo y el Modelo 2, la prueba de razón de verosimilitud es estadísticamente significativa, $\chi^2$ (13) = 909.14 [394.56+514.58], p < .001, lo cual indica que un modelo con interceptos aleatorios que incorpora la posición de clase y la afiliación sindical a nivel individual, y la concentración del ingreso y el corporativismo a nivel contextual, se ajusta significativamente mejor a los datos que un modelo nulo o sin predictores. Además, tanto el AIC como el BIC son menores en el Modelo 2, lo cual representa un mejor ajuste. En cuanto a la comparación entre modelos que incluyen predictores, la diferencia de la devianza entre el Modelo 2 y el Modelo 3 es significativa $\chi^2$ (7) = 354.72, p < .001, por lo cual se puede determinar que un modelo que añade los predictores de control mejora sustantivamente el ajuste de la estimación. Así también, los valores del AIC y el BIC son menores en este modelo en comparación al Modelo 2.
