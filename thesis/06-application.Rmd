...
Para ilustrar esta relación, en la Figura \@ref(fig:fig10) se grafican los valores predichos de $PSCi$ para cada categoría de clase según los niveles mínimos y máximos de la concentración del ingreso.[^15] En términos sustantivos, cuatro elementos pueden notificarse. Primero, en países con baja desigualdad económica no existen mayores diferencias entre las posiciones de clase respecto a la percepción de conflicto ya que, como se observa, las variaciones en el patrón son mínimas. Segundo, en países con alta desigualdad económica si se pueden observar leves diferencias entre las categorías de clase respecto al valor esperado en la escala de $PSCi$. Particularmente, los pequeños empleadores, los expertos directivos y los expertos sin autoridad presentan diferencias significativas que constatan con los coeficientes del Modelo 4. Tercero, en contextos de alta desigualdad estas tres categorías de clase tienden a percibir menores conflictos sociales que los individuos pertenecientes a la clase capitalista o de grandes propietarios. Y cuarto, los individuos de clases subordinadas no perciben mayores conflictos que los individuos de clase capitalista cuando la desigualdad aumenta sino que, al contrario, perciben menores tensiones sociales.

### Ajuste global

En relación al ajuste de los modelos, en la Tabla 3.3 se presentan los estadísticos del análisis de devianza (-2 veces la razón de verosimilitud o log likelihood en inglés) incluyendo los modelos 1 al 4 y el modelo nulo o de base. La prueba de razón de verosimilitud se utiliza para comparar modelos anidados y determinar cuál se ajusta mejor a los datos. De acuerdo con esta, se puede notificar que los modelos más complejos incrementan significativamente el ajuste. Tomando como ejemplo el Modelo nulo y el Modelo 2, la prueba de razón de verosimilitud es estadísticamente significativa, $\chi^2$ (13) = 909.13 [394.55+514.57], p < .001, lo cual indica que un modelo con interceptos aleatorios que incorpora la posición de clase y la afiliación sindical a nivel individual, y la concentración del ingreso y el corporativismo a nivel contextual, se ajusta significativamente mejor a los datos que un modelo nulo o sin predictores. Así también, la diferencia de la devianza entre el Modelo 2 y el Modelo 3 es significativa $\chi^2$ (7) = 354.71, p < .001, por lo cual se puede determinar que un modelo que añade los predictores de control mejora sustantivamente el ajuste de la estimación. 

Por último, cabe preguntarse por si el Modelo 4, que incluye la interacción entre niveles y la pendiente aleatoria, ajusta mejor que el Modelo 3 que solo utiliza interceptos aleatorios ya que, como se observa en la Tabla 3.2, solo tres coeficientes de la interacción fueron significativos y tanto las medidas de ajuste del AIC como el BIC aumentaron. Los resultados de la Tabla 3.3 sugieren que, a pesar de lo anterior, un modelo que incorpora la interacción entre la posición de clase de los individuos y la concentración del ingreso de los países se ajusta significativamente mejor a los datos, $\chi^2$ (52) = 84.38, p < .01, que un modelo con interceptos aleatorios y que asume coeficientes de regresión constantes entre los países. Por tanto, y aunque la diferencia de la devianza es pequeña, se puede concluir que el Modelo 4 que incorpora tanto los efectos fijos de las variables de hipótesis y de control como la pendiente aleatoria y la interacción entre niveles representa la mejor estimación de las percepciones de conflicto social en los 33 países bajo estudio entre los años 1999-2019. 
...
res_anova <- res_anova[c(2,3,5,6,8)] %>% as.tibble(.)

# agregar un add column con nombre modelos
# recodificar la columna de signifcancia cambiarle el nombre y los valores ponerlos con < 0.01
```
