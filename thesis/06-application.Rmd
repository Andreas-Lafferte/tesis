...
En esta sección se presentan los resultados de los modelos multinivel para los 33 países bajo estudio entre los años 1999, 2009 y 2019. Siguiendo una estrategía de análisis que va desde lo simple a lo complejo, se calculan cinco modelos [@hoxMultilevelAnalysisTechniques2017a]. Como es usual, se comienza estimando un modelo nulo o de base (no mostrado) en el que solo se descompone la varianza de $PSCi$ a nivel individual y contextual con el fin de conocer si el conflicto percibido varía entre países. El Modelo 1 corresponde a un modelo con interceptos aleatorios en el que se incluyen las variables independientes de nivel individual, mientras que en el Modelo 2 se añaden las variables independientes de nivel contextual. En el Modelo 3 se replica la misma estimación que en el Modelo 2 pero se incorporan las variables de control. Por último, en el Modelo 4 se trabaja con las mismas variables que en el Modelo 3, sin embargo, se diferencia en que añade la estimación de la pendiente aleatoria y la interacción entre niveles. La sección finaliza reportando los estadísticos de bondad de ajuste de los modelos y se cierra con un resumen de los principales hallazgos.

En la Tabla \@ref(tab:model) se presentan los resultados de los modelos multinivel. La correlación intraclase (ICC) para el modelo nulo es igual a 0.217, lo cual indica la cantidad de varianza de $PSCi$ que puede atribuirse a la estructura de agrupación en la población, en este caso, los países [ICC en base a @hoxMultilevelAnalysisTechniques2017a]. Esto significa que cerca de un 22% de la varianza total del conflicto percibido por los individuos se asocia a características específicas de los países, asentando como precedente que existen diferencias significativas entre estos respecto al nivel de percepción de conflicto.
...
capmo <- "Modelos de regresión multinivel para las percepciones de conflicto social"

texreg::htmlreg(l = list(model_2.e, model_3.e, model_4.e, model_6.e),
                    custom.model.names = c("Modelo 1", "Modelo 2", "Modelo 3", "Modelo 4"),
                    custom.coef.names = c("Intercepto", "2.Pequeños empleadores", "3.Pequeña burguesia", "4.Expertos directivos", "5.Expertos sin autoridad", "6.Supervisores calificados", "7.Supervisores no calificados", "8.Trabajadores calificados", "9.Trabajadores no calificados", "Miembro sindicato (ref: No)", "Ratio 80/20", "Corporativismo", "2009", "2019", "Edad (en años)", "Mujer (ref: Hombre)", "Centro", "Izquierda", "Sin identificación", "Gasto social (%GDP)", "Densidad sindical","2.Pequeños empleadores x Ratio 80/20", "3.Pequeña burguesia x Ratio 80/20", "4.Expertos directivos x Ratio 80/20", "5.Expertos sin autoridad x Ratio 80/20", "6.Supervisores calificados x Ratio 80/20", "7.Supervisores no calificados x Ratio 80/20", "8.Trabajadores calificados x Ratio 80/20", "9.Trabajadores no calificados x Ratio 80/20"),
                    groups = list("<b>Efectos fijos</b>" = 1, "Posición de clase (ref: 1.Capitalistas)" = 2:9,
                      "Tiempo (ref: 1999)" = 13:14,
                      "Identificación política (ref: Derecha)" = 17:19, "<b>Interacciones entre niveles</b> (ref: 1.Capitalistas x Ratio 80/20)" = 22:29),stars = c(0.01,0.05,0.1), custom.gof.rows = list("<b>Efectos aleatorios</b>" = c(" ", " ", " ", " "), "<b>Ajuste modelo</b>" = c(" ", " ", " ", " "), "$\\tau_{00}$ (varianza nivel país)" = c("0.89", "2.23", "2.49", "2.67"), "$\\sigma^2$ (varianza nivel individual)" = c("3.26", "3.22", "3.19", "3.18")),
