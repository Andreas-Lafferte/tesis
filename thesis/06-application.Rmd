...
tab1 <- tbl_stack(tbls = list(t1, t2)) %>% 
  ...
  pack_rows("Nivel contextual (país-año)", 28, 42)
...
texreg::htmlreg(l = list(model_2.e, model_3.e, model_4.e),
                    ...
                    custom.coef.names = c("Intercepto", "2.Pequeños empleadores", "3.Pequeña burguesia", "4.Expertos directivos", "5.Expertos sin autoridad", "6.Supervisores calificados", "7.Supervisores no calificados", "8.Trabajadores calificados", "9.Trabajadores no calificados", "Miembro sindicato (ref: No)", "Ratio 80/20", "Corporativismo", "2009", "2019", "Edad (en años)", "Mujer (ref: Hombre)", "Centro", "Izquierda", "Sin identificación", "Gasto social (%GDP)", "Densidad sindical"),single.row = T,
                    groups = list("<b>Efectos fijos</b>" = 1, "Posición de clase (ref: 1.Capitalistas)" = 2:9,
                      "Tiempo (ref: 1999)" = 13:14,
                      "Identificación política (ref: Derecha)" = 17:19),stars = c(0.01,0.05,0.1), custom.gof.rows = list("<b>Efectos aleatorios</b>" = c(" ", " ", " "), "<b>Ajuste modelo</b>" = c(" ", " ", " "), "$\\tau_{00}$ (varianza nivel país)" = c("0.89", "2.23", "2.49"), "$\\sigma^2$ (varianza nivel individual)" = c("3.26", "3.22", "3.19")),
...
Por último, distintos elementos pueden destacarse en relación a las variables de control de nivel contextual. Primero, es importante mencionar que en estos modelos no se incluyó el GDP per capita de los países debido que presentaba serios problemas de multicolinealidad con la variable tiempo (ver Figura \@ref(fig:fig11) del [Apéndice](#apendiceb)). Segundo, el Modelo 3 da cuenta de que el gasto social de los países posee un efecto negativo y estadísticamente significativo ($\beta$ = -0.05 , SE = 0.01, p < .01), lo cual es consistente con lo encontrado en investigaciones anteriores. Tercero, el efecto de la densidad sindical de los países es positivo y significativo ($\beta$ = 0.02 , SE = 0.00, p < .01), lo cual quiere decir que a mayor densidad sindical de los países mayores tienden a ser las percepciones de conflicto social en los individuos. Este último hallazgo establece un desafío a las investigaciones comparadas puesto que, contrario a lo consensuado, pareciera ser que en países con mayor poder sindical se estarían desarrollando percepciones más orientadas hacia el conflicto. 

### Interacción entre niveles

Como se ha demostrado, tanto la posición de clase de los individuos como el nivel de desigualdad económica de los países tienen un efecto significativo y positivo en la escala de percepción de conflictos. Ahora, resta examinar la hipótesis sobre que la relación entre la posición de clase y el conflicto percibido puede variar en función de (*i.e.* es moderada por) el nivel de desigualdad económica de los países.[^14] 
...
texreg::htmlreg(l = list(model_6.e),
                    ...
                    custom.coef.names = c("Intercepto", "2.Pequeños empleadores", "3.Pequeña burguesia", "4.Expertos directivos", "5.Expertos sin autoridad", "6.Supervisores calificados", "7.Supervisores no calificados", "8.Trabajadores calificados", "9.Trabajadores no calificados", "Miembro sindicato (ref: No)", "Ratio 80/20", "Corporativismo", "2009", "2019", "Edad (en años)", "Mujer (ref: Hombre)", "Centro", "Izquierda", "Sin identificación", "Gasto social (%GDP)", "Densidad sindical","2.Pequeños empleadores $\\times$ Ratio 80/20", "3.Pequeña burguesia $\\times$ Ratio 80/20", "4.Expertos directivos $\\times$ Ratio 80/20", "5.Expertos sin autoridad $\\times$ Ratio 80/20", "6.Supervisores calificados $\\times$ Ratio 80/20", "7.Supervisores no calificados $\\times$ Ratio 80/20", "8.Trabajadores calificados $\\times$ Ratio 80/20", "9.Trabajadores no calificados $\\times$ Ratio 80/20"),
                ...
                reorder.gof = c(1,2,5,4,9,10,3,6,7,8),
                    ...
                    caption.above = T,
                custom.note = "\n**Nota**: Estimación de máxima verosimilitud restringida, coeficientes no estandarizados. Errores estándar entre paréntesis (SE). Variables de nivel contextual centradas a sus grandes medias (CGM). Efectos aleatorios del Modelo 4 no mostrados por parsimonia. Modelo 4 controla por las variables utilizadas en los modelos anteriores. Datos ponderados. <br>  $^{***}$ p < 0.01; $^{**}$ p < 0.05; $^{*}$ p < 0.1")

```
<br/>


En la Tabla \@ref(tab:model2) se muestran los resultados de las interacciones entre niveles. De acuerdo con el Modelo 4, la $H_{3b}$ que establecía que a mayores niveles de desigualdad económica los individuos de clases subordinadas aumentarían su percepción de conflicto social y los individuos de clases privilegiadas la mantendrían o disminuirían, debe ser rechazada. En términos generales, no existe evidencia que sustente lo planteado hipotéticamente ya que, por un lado, todos los coeficientes de las categorías de clase son negativos, lo cual indica que a mayor concentración del ingreso las clases más bajas/subordinados perciben menos conflicto que los individuos de clase capitalista y, por el otro, solo tres categorías de clase muestran coeficientes estadísticamente significativos. En específico, los resultados de la interacción dan cuenta que, por ejemplo, por cada unidad que aumenta la concentración del ingreso, el efecto de pertenecer a la clase de los expertos directivos disminuye en -0.04 puntos en la escala de $PSCi$ en comparación a los individuos de clase capitalista, una disminución significativa al 99% de confianza. Similarmente, en países con mayor desigualdad económica, los expertos sin autoridad ($\beta$ = -0.03, SE = 0.01, p < .05) y los pequeños empleadores ($\beta$ = -0.03, SE = 0.01, p < .1) perciben menores (no mayores) conflictos sociales que los individuos de clase capitalista. Por su parte, las posiciones de clase subordinadas -como los trabajadores no calificados, calificados y supervisores no calificados-, no muestran coeficientes significativos y, más allá de este hecho, la dirección de los efectos es negativa. 
...
g7 <- ggpredict(model_6, terms = c("CLASS", "C_RATIO [minmax]")) %>% 
...
Para ilustrar esta relación, en la Figura \@ref(fig:fig10) se grafican los valores predichos de $PSCi$ para cada categoría de clase según los niveles mínimos y máximos de la concentración del ingreso.[^15] En términos sustantivos, cuatro elementos pueden notarse. Primero, en países con baja desigualdad económica no existen mayores diferencias entre las posiciones de clase respecto a la percepción de conflicto ya que, como se observa, las variaciones en el patrón son mínimas. Segundo, en países con alta desigualdad económica si se pueden observar leves diferencias entre las categorías de clase respecto al valor esperado en la escala de $PSCi$. Particularmente, los pequeños empleadores, los expertos directivos y los expertos sin autoridad presentan diferencias significativas que coinciden con los coeficientes del Modelo 4. Tercero, en contextos de alta desigualdad estas tres categorías de clase tienden a percibir menores conflictos sociales que los individuos pertenecientes a la clase capitalista. Y cuarto, los individuos de clases subordinadas no perciben mayores conflictos que los individuos de clase capitalista cuando la desigualdad aumenta sino que, al contrario, perciben menos tensiones sociales.

### Ajuste comparativo

En relación al ajuste de los modelos, en la Tabla \@ref(tab:fit) se presentan los estadísticos del análisis de devianza (likelihood ratio test) incluyendo los modelos 1 al 4 y el modelo nulo. La prueba de razón de verosimilitud se utiliza para comparar modelos anidados y determinar cuál se ajusta mejor a los datos. De acuerdo con esta prueba, se puede advertir que los modelos más complejos incrementan significativamente el ajuste. Tomando como ejemplo el Modelo nulo y el Modelo 2, la prueba de razón de verosimilitud es estadísticamente significativa, $\chi^2$ (13) = 909.14 [394.56+514.58], p < .001, lo cual indica que un modelo con interceptos aleatorios que incorpora la posición de clase y la afiliación sindical a nivel individual, y la concentración del ingreso y el corporativismo a nivel contextual, se ajusta significativamente mejor a los datos que un modelo nulo o sin predictores. Además, tanto el AIC como el BIC son menores en el Modelo 2, lo cual representa un mejor ajuste. En cuanto a la comparación entre modelos que incluyen predictores, la diferencia de la devianza entre el Modelo 2 y el Modelo 3 es significativa $\chi^2$ (7) = 354.72, p < .001, por lo cual se puede determinar que un modelo que añade los predictores de control mejora sustantivamente el ajuste de la estimación. Así también, los valores del AIC y el BIC son menores en este modelo en comparación al Modelo 2.
